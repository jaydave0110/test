...
@endsection 
<div class="modal" id="loadingPopup" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">Hang-on!</div>
      <div class="modal-body">
        <h1 class="text-center"><i class="fa fa-spin fa-spinner"></i> Please wait</h1>
      </div>
    </div>
  </div>
</div>
            ...
            $.ajax({
                ...
                success: function(xhr, textStatus, jQxhr) {
                    ...
                    if (typeof $("select#" + ajaxCity).attr('multiple') == 'undefined') {
                        $("select#" + ajaxCity).append('<option value="">Select City</option>');
                    }
...
<script >
    ...
    $(document).on('click', '.deleteSiteImage', function(e) {
        ...
        if (typeof imageId != 'undefined' && imageId > 0) {
            if (confirm('Are you sure you want to delete this image')) {
                $.ajax({
                    url: '{{ url("/deleteBuilderImage")}}' +'/'+imageId,
                    method: 'GET',
                    data: $(this).serialize(),
                    beforeSend: function(data) {
